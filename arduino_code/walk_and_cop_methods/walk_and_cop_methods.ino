...
const int wSwingSpeed = 14;
const int stairSwingSpeed = 10;
const int mindel = 0;
...
//    prevpin4 = 0;
    ...
    if (state != prevState){
      swingPulse(rfa,rba,rbw,rfw,lh,wSwingSpeed);
    }
  }else if(state == 25){ // // hybrid walking left swing
    if (state != prevState){
      swingPulse(lfa,lba,lbw,lfw,rh,wSwingSpeed);      
    }
  }else if(state == 16){ // stairs right swing
    if (state != prevState){
      swingPulse(rfa,rba,rbw,rfw,lh,stairSwingSpeed);
    }
  }else if(state == 26){ // stairs left swing
    if (state != prevState){
      swingPulse(lfa,lba,lbw,lfw,rh,stairSwingSpeed);   
    }
                                                                   // COP
